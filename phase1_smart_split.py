print("╔════════════════════════════════════════════════════╗")
print("║   AJX ULTIMATE: TARGETED ZOOM SCAN                 ║")
print("║   (Intense Scan ONLY on Index Pages specified)     ║")
print("╚════════════════════════════════════════════════════╝")
...
# --- TRIPLE-CHECK SCANNER (Runs ONLY on Index Pages) ---
def scan_page_with_zoom(image):
    """
    Scans a single page in 3 passes: Full, Top Half, Bottom Half.
    """
    width, height = image.size
    top_half = image.crop((0, 0, width, int(height * 0.6)))
    bottom_half = image.crop((0, int(height * 0.4), width, height))
    
    scans = [
        {"img": image, "type": "FULL_PAGE"},
        {"img": top_half, "type": "TOP_HALF"},
        {"img": bottom_half, "type": "BOTTOM_HALF"}
    ]
    
    combined_chapters = []
    
    prompt = """
    Extract all chapters and page numbers.
    Layout is variable (Columns/Grid). FIND EVERYTHING.
    Output JSON: [{"chapter_name": "Name", "start_page": 5}]
    """
    
    for scan in scans:
        try:
            response = model.generate_content([prompt, scan['img']])
            text = response.text.replace("```json", "").replace("```", "").strip()
            data = json.loads(text)
            
            if isinstance(data, list):
                for item in data:
                    item['start_page'] = clean_page_num(item.get('start_page', 0))
                    if item['start_page'] > 0:
                        combined_chapters.append(item)
        except:
            pass
...
def generate_index_from_range(pdf_name, input_str, total_pages, book_folder_id, json_name):
    ...
    log(f"\n🏗️ MODE 2: BUILDING INDEX FROM RANGE '{input_str}' (TRIPLE PASS)...")
    
    # 1. PARSE RANGE (e.g., "25-27" -> [25, 26, 27])
    pages_to_read = []
    if "-" in input_str:
    ...
    # 2. CONVERT ONLY THESE SPECIFIC PAGES
    log(f"   -> Scanning ONLY pages {chunk_start} to {chunk_end} at 300 DPI...")
    images = convert_from_path(pdf_name, first_page=chunk_start, last_page=chunk_end, dpi=300)
    
    all_raw_data = []
    
    # 3. RUN TRIPLE-CHECK (Only on the Index Pages)
    for i, img in enumerate(images):
        page_num = chunk_start + i
        log(f"   -> 🔍 Triple-Checking Page {page_num}...")
        page_data = scan_page_with_zoom(img)
        all_raw_data.extend(page_data)
        log(f"      -> Found {len(page_data)} items on this page.")

    # 4. MERGE & DEDUPLICATE
    log("Cc Merging and Deduplicating...")
    unique_map = {}
    for item in all_raw_data:
        p = item['start_page']
        name = item.get('chapter_name', 'Unknown')
        
        if p not in unique_map:
            unique_map[p] = item
        else:
            if len(name) > len(unique_map[p]['chapter_name']):
                unique_map[p] = item

    toc_data = list(unique_map.values())
    ...
    for i, chap in enumerate(final_toc):
        start_p = chap['start_page']
        if i < len(final_toc) - 1:
            next_p = final_toc[i+1]['start_page']
            end_p = max(start_p, next_p - 1)
        else:
            end_p = total_pages
        chap['end_page'] = end_p
        chap['range'] = f"{start_p}-{end_p}"

    with open(json_name, 'w', encoding='utf-8') as f:
        json.dump(final_toc, f, indent=4, ensure_ascii=False)
...
def main():
    ...
    # --- FAST CONVERSION (Rest of the book) ---
    log("\n🚀 STARTING IMAGE CONVERSION (FAST MODE)...")
    try:
        info = pdfinfo_from_path(pdf_name)
        total_pages = int(info["Pages"])
    except:
